...
(function($) {
	...
	$.fn.Framer = function(settings) {
		...
		// windowã‚µã‚¤ã‚ºã‹ã‚‰ã€#framerã«åˆ©ç”¨å¯èƒ½ãªã‚µã‚¤ã‚ºã‚’è¨ˆç®—ã€‚è¨­å®š
		var setBoxSize = function(contents) {
			...
			// #framerContainer ã®borderã¨paddingã®ã‚µã‚¤ã‚ºã‚’å–å¾—ã™ã‚‹ã€‚ã“ã®æ™‚ç‚¹ã§ã¯widthã€heightã¨ã‚‚ã«0ãªã¯ãšã®ãªã®ã§ãƒ»ãƒ»ãƒ»
			...
			// #framerContainerã®paddingã€border-sizeã¨ã‚³ãƒ³ãƒ†ãƒ³ãƒ„ã®ã‚µã‚¤ã‚ºã‚’è¶³ã—ãŸæ•°å€¤ã€‚ã“ã‚ŒãŒ#framerã®ã‚µã‚¤ã‚ºã¨ãªã‚‹ãŸã‚ã€ç¸®å°ã™ã‚‹å ´åˆã®ãƒ™ãƒ¼ã‚¹ã®å€¤ã¨ãªã‚‹
			...
			if(mw > mh) {	// ç¸¦ã‚¹ãƒšãƒ¼ã‚¹ãŒæ¨ªã‚¹ãƒšãƒ¼ã‚¹ã‚ˆã‚Šå°ã•ã„
				if(wh * settings.resizeRatio < targetHeight) {
					// ãƒªã‚µã‚¤ã‚ºå‡¦ç†
					if(settings.isAutoResize) {
						// console.log("hhhh", targetHeight);
						// ã¾ãšã¯ä¸€ç•ªè¦ªã®ã‚³ãƒ³ãƒ†ãƒŠã¨ãªã‚‹#framerã®ã‚µã‚¤ã‚ºã‚’è¨­å®šã€‚ã“ã®æ™‚ã€#framerContainerã®padding.border-sizeã‚’å«ã‚“ã ã‚µã‚¤ã‚º
						FRM.box.height(wh * settings.resizeRatio);	// Framerã¸ã®paddingã‚’è€ƒæ…®ã«å…¥ã‚ŒãŸæ•°å€¤
						ratio = FRM.box.height() / targetHeight;
						FRM.box.width(targetWidth * ratio);

						// #framerã«è¨­å®šã•ã‚ŒãŸã‚µã‚¤ã‚ºã‹ã‚‰ã€#framerContainerã®paddingã€border-sizeã‚’å¼•ã„ãŸã‚µã‚¤ã‚ºã‚’ã‚³ãƒ³ãƒ†ãƒ³ãƒ„ã«è¨­å®š
			...
			else {	// æ¨ªã‚¹ãƒšãƒ¼ã‚¹ãŒç¸¦ã‚ˆã‚Šå°ã•ã„
		...
		var setContentsSize = function(contents, w, h) {
			// æ¸¡ã•ã‚Œã‚‹å€¤ã¯ã€ç”»é¢ã‚µã‚¤ã‚ºã‹ã‚‰è¨ˆç®—ã•ã‚ŒãŸcontentéƒ¨åˆ†ã«åˆ©ç”¨å¯èƒ½ãªã‚µã‚¤ã‚ºãªã®ã§ã€ã“ã“ã§contentè‡ªä½“ã«è¨­å®šã•ã‚Œã¦ã„ã‚‹paddingã€borderã‚’å¼•ã„ãŸã‚µã‚¤ã‚ºã‚’è¨­å®šã™ã‚‹ã€‚

			var contentsWidth = contents.width();
			var contentsHeight = contents.height();

			if(contentsWidth === 0) {
				// ãã‚‚ãã‚‚contentsã®ã‚µã‚¤ã‚ºã‚’å–å¾—ã§ããªã‹ã£ãŸã‚‰ã€ãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿ã®å€¤ãã®ã¾ã¾è¨­å®šã™ã‚‹
				contents.width(w);
			}
			else {
